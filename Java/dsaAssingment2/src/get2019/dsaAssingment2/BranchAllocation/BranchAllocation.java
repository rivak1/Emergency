...
public class BranchAllocation {
	...
	/**
	 * this class is used to read excel file and convert that data into Object form 
	 * @param StudentDetails this all are list type which contain student type object
	 * @param BranchPrefrence this contain all branch prefrence of student
			...
			for (int j = 0; j < perticulatStudentBranch.size(); j++) {
				...
				for (int k = 0; k < Obj1.Programs.size(); k++) {
					if (studentBranch.equals(Obj1.Programs.get(k).branch)) {
						if (Obj1.Programs.get(k).capacity != 0) { //if branch capacity is not 0 then allocated the branch
